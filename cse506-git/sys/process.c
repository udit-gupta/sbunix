...
//    print("before");
 ...
 //   print("stack=%x",pcb->kstack[510]);
    ...
    lcr3(boot_cr3);
  //  print("pcb->entry=%x",(uint64_t)pcb->entry);
/*
...
void schedule() {
   ...
   // int i;
    ...
    if(c==100) {
        ...
        /*   for(i=1;i<=process_count;i++) {    
        //pcb = (struct task_struct *)ready_queue[i]; 
        if(( (struct task_struct *)ready_queue[i] )->sleep_time > 0)
        {
            (( (struct task_struct *)ready_queue[i] )->sleep_time)--;    
            print("%d", ( (struct task_struct *)ready_queue[i] )->sleep_time);
           // asm("hlt");
        }
    }
*/
    ...
    //    print("inside first switch");
        ...
        int k=1;                                                                                                                                  while(k<=process_count)  // find the first ready process wHo is not sleeping and not in wait 
        {
            ...
            if((pcb->sleep_time == 0) /*&& (pcb->is_waiting==0)*/ )
                break;
                k++;
       } 

  /*    while(k<=process_count)  // find the first ready process wHo is not sleeping 
        {
            //pcb =(struct task_struct *) ready_queue[k];
            if(( (struct task_struct *)ready_queue[k] )->sleep_time == 0) 
                break;
            k++;
        } 
...
//        print("%d", k);
  ...
  //  print("\nSchedule(IF) - pcb->pid=%d, process_count=-%d,j=%d\n",next->pid,process_count,j);
  ...
  //      int i;
   ...
   // print("\nSchedule Else(prev) - pcb->pid=%d, process_count=-%d,j=%d\n",next->pid,process_count,j);
  //    if(j == 0 )   // we don't do switch with kernel now 
  //     j = 1; 
      /* for(i=1;i<=process_count;i++)    // find the first awaken process in the round robin queue 
       {
        print("\n\nj = %d time %d rip %x", j, ( (struct task_struct *)ready_queue[i] )->sleep_time, ( (struct task_struct *)ready_queue[i] )->rsp  );
       */  //pcb = (struct task_struct *)ready_queue[j];
        // print("\n time %d", pcb->sleep_time);
   /*      if(( (struct task_struct *)ready_queue[i] )->sleep_time == 0 && prev->pid != ( (struct task_struct *)ready_queue[i] )->pid)
          break;
  ...
  //    print("\nSchedule Else (next) - pcb->pid=%d, process_count=-%d,j=%d\n",next->pid,process_count,j);
      
//       print("<<PCB->PID=%d,process_count=%d,j=%d>>",next->pid,process_count,j); 
      __asm__ __volatile__(
                "movq %%rsp, %0;"
                :"=m"(prev->rsp)
                :   
...
//     print("Else - I am in process virtual address space \n");
     //tss.rsp0 =(uint64_t)(&(next->kstack[511]));    
     tss.rsp0 =(uint64_t)(&(next->kstack[511]));    
    /* 
     if(cont) {  
         asm volatile("addq $0x8,%rsp");
         asm volatile("addq $0x8,%rsp");
     }
...
int fork()
{
   ...
   */ //for(i=0;i<30;i++) 
    ...
    //asm volatile("hlt");
    // printf("\n");
  //  child_ret_addr = rsp1[19];
 //   child_stack_addr = rsp1[22];
...
**************************************************************************/
/*uint64_t execvpe(uint64_t arg1,uint64_t arg2, uint64_t arg3)
{
   ...
   // int i=0;
    ...
    print("done");
    struct task_struct *pcb = malloc_pcb("bin/test");
    print("done");
